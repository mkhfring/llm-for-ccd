...
class ChatGPTRequest:
    ...
    def process_prompts(self, prompts):
        requested_ids = get_requested_ids(os.path.join(current_location, 'last_id_ruby_java2.txt'))
        for prompt in prompts:
            sample_id = prompt[0]
            if sample_id in requested_ids:
                continue
            
            with open(os.path.join(current_location, 'last_id_ruby_java2.txt'), 'a') as file:
            ...
            with open(os.path.join(current_location, 'results_for_java_ruby2.txt'), 'a') as file:
                file.write(f"***Data Id {sample_id}: {result['choices'][0]['message']['content'].strip()}+++\n \n")

def main():
    chat_gpt = ChatGPTRequest()
    data = read_data(os.path.join(current_location, 'ruby_java_test_clone2.jsonl'))
