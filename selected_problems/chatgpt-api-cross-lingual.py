...
class ChatGPTRequest:
    ...
    def process_prompts(self, prompts):
        requested_ids = get_requested_ids(os.path.join(current_location, 'last_id_ruby_java.txt'))
        for prompt in prompts:
            sample_id = prompt[0]
            if sample_id in requested_ids:
                continue
            
            with open(os.path.join(current_location, 'last_id_ruby_java.txt'), 'a') as file:
            ...
            with open(os.path.join(current_location, 'results_for_java_ruby.txt'), 'a') as file:
